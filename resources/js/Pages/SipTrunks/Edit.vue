...
const settings = props.sipTrunk.settings || {}
const credentials = props.sipTrunk.credentials || {}
...
const form = useForm({
  ...
  webhook_failover_url: settings.webhook_failover_url || '',
  webhook_api_version: settings.webhook_api_version || '',
  webhook_timeout_secs: settings.webhook_timeout_secs ? String(settings.webhook_timeout_secs) : '',
  notes: props.sipTrunk.notes || '',
  user_name: credentials.user_name || '',
  password: credentials.password || '',
  anchorsite_override: settings.anchorsite_override || '',
  sip_uri_calling_preference: settings.sip_uri_calling_preference || '',
  dtmf_type: settings.dtmf_type || '',
  encrypted_media: settings.encrypted_media || '',
  noise_suppression: settings.noise_suppression || '',
  onnet_t38_passthrough_enabled: settings.onnet_t38_passthrough_enabled ?? true,
  default_on_hold_comfort_noise_enabled: settings.default_on_hold_comfort_noise_enabled ?? false,
  encode_contact_header_enabled: settings.encode_contact_header_enabled ?? true,
  third_party_control_enabled: settings.third_party_control_enabled ?? false,
  inbound_ani_format: settings.inbound_ani_format || '',
  inbound_dnis_format: settings.inbound_dnis_format || '',
  inbound_routing_method: settings.inbound_routing_method || '',
  inbound_channel_limit: settings.inbound_channel_limit ? String(settings.inbound_channel_limit) : '',
  inbound_codecs: toArray(settings.inbound_codecs),
  inbound_ringback_tone: settings.inbound_ringback_tone ?? true,
  inbound_instant_ringback: settings.inbound_instant_ringback ?? false,
  inbound_isup_headers: settings.inbound_isup_headers ?? true,
  inbound_prack: settings.inbound_prack ?? true,
  inbound_sip_compact_headers: settings.inbound_sip_compact_headers ?? true,
  inbound_simultaneous_ringing: settings.inbound_simultaneous_ringing
    ? String(settings.inbound_simultaneous_ringing)
    : '',
  inbound_timeout_1xx: settings.inbound_timeout_1xx ? String(settings.inbound_timeout_1xx) : '',
  inbound_timeout_2xx: settings.inbound_timeout_2xx ? String(settings.inbound_timeout_2xx) : '',
  inbound_shaken_stir: settings.inbound_shaken_stir ?? true,
  outbound_call_parking: settings.outbound_call_parking ?? true,
  outbound_ani_override: settings.outbound_ani_override || '',
  outbound_ani_override_type: settings.outbound_ani_override_type || '',
  outbound_channel_limit: settings.outbound_channel_limit ? String(settings.outbound_channel_limit) : '',
  outbound_instant_ringback: settings.outbound_instant_ringback ?? false,
  outbound_ringback_tone: settings.outbound_ringback_tone ?? true,
  outbound_localization: settings.outbound_localization || '',
  outbound_codecs: toArray(settings.outbound_codecs),
  outbound_t38_reinvite_source: settings.outbound_t38_reinvite_source || '',
  rtcp_port: settings.rtcp_port || '',
  rtcp_capture_enabled: settings.rtcp_capture_enabled ?? true,
  rtcp_report_frequency: settings.rtcp_report_frequency ? String(settings.rtcp_report_frequency) : '',
  ios_push_credential_id: settings.ios_push_credential_id || '',
  android_push_credential_id: settings.android_push_credential_id || '',
  outbound_voice_profile_id: settings.outbound_voice_profile_id
    ? String(settings.outbound_voice_profile_id)
    : '',
  phone_numbers:
    props.sipTrunk.phone_numbers?.map((pn) => ({
      phone_number_id: pn.id ? String(pn.id) : '',
...
const numberOptions = computed(() => {
  const map = new Map()
  ;(props.sipTrunk.phone_numbers || []).forEach((pn) => {
    if (pn && pn.id) {
      map.set(pn.id, pn)
    }
  })
  ;(props.availablePhoneNumbers || []).forEach((pn) => {
    if (pn && pn.id && !map.has(pn.id)) {
      map.set(pn.id, pn)
    }
  })
...
const submit = () => {
  form
    ...
    .put(route('sip-trunks.update', props.sipTrunk.id), {
      preserveScroll: true,
      onError: (errors) => {
        if (errors.telnyx) {
...
<template>
  <DashboardLayout>
    <Head :title="`Edit SIP Trunk · ${props.sipTrunk.name}`" />
      ...
      <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p class="text-sm text-muted-foreground">
            Update routing, media resilience, and webhook targets for
            <span class="font-semibold text-foreground">{{ props.sipTrunk.name }}</span>.
          </p>
        </div>
        <Link
          :href="route('sip-trunks.show', props.sipTrunk.id)"
          class="inline-flex items-center gap-2 text-sm text-muted-foreground transition hover:text-foreground"
        >
          <ArrowLeft class="h-4 w-4" />
          Back to trunk details
        </Link>
      </div>
    </template>
        ...
        <div class="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Core metadata and webhook endpoints.</CardDescription>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Credentials used when registering endpoints against Telnyx.</CardDescription>
            </CardHeader>
            <CardContent class="grid gap-4 md:grid-cols-2">
              ...
              <div class="space-y-2">
                ...
                <Input
                  ...
                  type="password"
                  placeholder="Update credential (leave blank to keep current)"
                />
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Control how inbound calls are routed and formatted.</CardDescription>
            </CardHeader>
            <CardContent class="space-y-6">
              <div class="grid gap-4 md:grid-cols-2">
                <div class="space-y-2">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Localisation, caller ID policies, and codec preferences.</CardDescription>
            </CardHeader>
            <CardContent class="space-y-6">
              <div class="grid gap-4 md:grid-cols-3">
                <div class="space-y-2">
        ...
        <div class="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Manage DID inventory routed through this trunk.</CardDescription>
            </CardHeader>
            <CardContent>
              <div v-if="numberOptions.length || form.phone_numbers.length" class="space-y-4">
                <div
              ...
              <div v-else class="flex items-center gap-3 rounded-lg border border-dashed border-border px-4 py-6 text-sm text-muted-foreground">
                ...
                No additional phone numbers are available to assign right now.
              </div>
            </CardContent>
          </Card>
        </div>
